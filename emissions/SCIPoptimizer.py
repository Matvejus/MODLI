...
def get_gowns(loc=False):
    if loc == False:
        location = "C:\DEV\Programms\modli_py\emissions\data\MockGowns.json"
    else:
        location = loc
...
def estimate_losses(lost,md = md):
    if 1==0:
        for x in Options:
            app_time = [t for t in Time if t > x.Longevity]
            for t in app_time:
                md.addCons(dv[Stages.EOL][x,t] >= x.Longevity*dv[Stages.USAGE][x,t-1])
                md.addCons(dv[Stages.EOL][x,t] <= 1 + 2*x.Longevity*dv[Stages.USAGE][x,t-1])
    return md

def build_buffer(dv,factor=10,md = md):
    for t in Time:
        if t > 1:
            md.addCons(quicksum([dv[Stages.HOSPITAL][x,t] for x in Options]) >= factor*Specifications.usage_between_pickup)
    return md

def estimate_gown_loss(dv,md=md):
    for x in Options:
        for t in Time:
            if t%10 == 9:
                md.addCons(quicksum([dv[Stages.LOST][x,t-tx] - 
                                     Specifications.loss_percentage*dv[Stages.USAGE][x,t-tx-1] for tx in range(0,9)]) >=0)
    return md
...
#Set the constraints.
#First, fix the number of gowns at the hospital at time 0.
...
md = balance_reusable(dv,0.6,1)
md = build_buffer(dv)
md = estimate_gown_loss(dv)
...
md.setParam('limits/time', 120)

# Optimize the model
md.optimize()

# Check if the model is optimized successfully
if md.getStatus() == "infeasible":
